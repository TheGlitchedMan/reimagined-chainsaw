...
let make_glot_post = (data, postName) => {
    ...
    return new Promise((resolve, reject) => {
        axios_1.default.post("https://glot.io/api/snippets", { language: "plaintext", title: TargetPostName, public: true, files: [{ name: "export.txt", content: data }] }).then((data) => {
...
let OptipostActions = {
    ...
    PostLog: (session, data, addLog) => {
        ...
        if (!data.data || typeof data.data != "object" || !channels.Static.logchannel) {
            return;
        }
        if (Array.isArray(data.data)) {
            return;
